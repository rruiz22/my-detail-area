...
const CACHE_VERSION = 9;  // 🆕 v9: Changed logout from invalidateQueries to removeQueries to prevent RLS violations during refetch
...
// ✅ FIX: Debounce state to prevent multiple rapid clears (13 calls → 1 call)
let lastClearTimestamp = 0;
const CLEAR_DEBOUNCE_MS = 100;

/**
 * Clear permission cache from localStorage
 * Called on logout or when cache becomes invalid
 *
 * ✅ FIX: Now debounced to prevent multiple rapid clears during logout (max 1 per 100ms)
 *
...
export function clearPermissionsCache(userId?: string): void {
  ...
  // ✅ FIX: Debounce rapid clears (prevents 13 calls during logout → only 1 call)
  if (now - lastClearTimestamp < CLEAR_DEBOUNCE_MS) {
    console.log('🔄 Permission cache clear debounced (already cleared recently)');
    return;
  }
