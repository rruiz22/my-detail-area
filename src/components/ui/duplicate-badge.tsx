...
export function DuplicateBadge({ count, className, inline = true }: DuplicateBadgeProps) {
  // Don't render badge if no duplicates
  if (count <= 1) return null;

  const colors = getDuplicateBadgeColor(count);

  // Inline badge (new default)
  if (inline) {
    return (
      <span
        className={cn(
          "inline-flex items-center justify-center px-1.5 py-0 rounded text-[10px] font-bold leading-none shadow-sm border",
          colors.bg,
          colors.text,
          colors.border,
          className
  ...
  // Legacy absolute positioned badge (kept for backwards compatibility)
  return (
    <div
      className={cn(
