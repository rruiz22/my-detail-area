...
-- First check and fix the user_type enum
...
-- Alternative approach: Use the existing admin type or create a workaround
-- Let's first see what user_type values exist
-- Common values might be: 'dealer', 'detail', 'admin', 'dealer_admin'
...
-- Policy 2: System admins can see all profiles (using email as backup)
  ...
  USING (
    EXISTS (
      ...
      AND (
        ...
        OR p.user_type = 'admin'::user_type
        OR p.user_type = 'dealer_admin'::user_type
...
CREATE POLICY "profiles_select_dealer_admin" ON profiles
  ...
  USING (
    EXISTS (
      ...
      AND admin_profile.user_type IN ('admin', 'dealer_admin')
      AND EXISTS (
...
CREATE POLICY "profiles_insert_system_admin" ON profiles
  ...
  WITH CHECK (
    EXISTS (
      ...
      WHERE p.id = auth.uid() 
      AND (
        p.user_type IN ('system_admin', 'admin', 'dealer_admin')
        OR p.email = 'rruiz@lima.llc'
      )
    )
  );
...
-- Try to set the user type to system_admin if it exists, otherwise use available admin type
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_enum 
  ...
  ELSIF EXISTS (
    SELECT 1 FROM pg_enum 
    WHERE enumlabel = 'dealer_admin' 
    AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'user_type')
  ) THEN
    UPDATE profiles 
    SET user_type = 'dealer_admin'::user_type
    WHERE email = 'rruiz@lima.llc';
  END IF;
END $$;
...
SELECT 'Current user type for rruiz@lima.llc:' as info;
