...
// Face detection utilities (optional - can be toggled)
import { loadFaceDetectionModels, detectFace, areModelsLoaded } from "@/utils/faceDetection";
// Photo fallback utilities (PHASE 5 - optional manual capture)
import { capturePhotoFromVideo, uploadPhotoToStorage, checkStorageBucketExists } from "@/utils/photoFallback";
...
const PunchClockKiosk = () => {
  ...
  // Face detection states (NEW - doesn't affect existing functionality)
  const [useFaceDetection, setUseFaceDetection] = useState(false); // Toggle between simulated and real
  const [modelsLoaded, setModelsLoaded] = useState(false);
  const [faceDetectionStatus, setFaceDetectionStatus] = useState<string>("Loading models...");
  const detectionIntervalRef = useRef<number | null>(null);

  // Photo fallback states (PHASE 5 - optional manual photo capture)
  const [showPhotoCapture, setShowPhotoCapture] = useState(false); // Show manual photo capture UI
  const [capturedPhoto, setCapturedPhoto] = useState<string | null>(null); // Base64 photo data
  const [photoUploadStatus, setPhotoUploadStatus] = useState<string>(""); // Upload feedback
  ...
  // Load face detection models on mount (OPTIONAL - only if enabled)
  useEffect(() => {
    const loadModels = async () => {
      ...
      try {
        ...
        setUseFaceDetection(false); // Fall back to simulated
      }
    };
