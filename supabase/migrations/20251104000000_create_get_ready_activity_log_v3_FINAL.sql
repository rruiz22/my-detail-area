...
-- Version: 3 FINAL (Fixed FK reference to profiles)
-- =====================================================
...
-- Drop table if exists (to recreate with correct FK)
DROP TABLE IF EXISTS public.get_ready_vehicle_activity_log CASCADE;

-- Create activity log table with correct FK to profiles
CREATE TABLE public.get_ready_vehicle_activity_log (
  -- Primary identifiers
  ...
  -- Actor information - FK to profiles instead of auth.users
...
DO $$
BEGIN
  RAISE NOTICE '✅ Get Ready Activity Log migration V3 FINAL completed successfully!';
  RAISE NOTICE '📊 Table: get_ready_vehicle_activity_log';
  RAISE NOTICE '🔗 FK to profiles: action_by → profiles(id)';
