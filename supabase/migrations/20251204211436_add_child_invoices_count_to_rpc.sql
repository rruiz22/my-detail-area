-- Migration: Add child_invoices_count to get_invoices_with_filters RPC
-- Problem: RPC was not returning child invoice count, causing reinvoice indicator to never display
-- Solution: Add subquery to count child invoices for each parent invoice
...
-- Recreate function with child_invoices_count field
...
RETURNS TABLE (
  ...
  child_invoices_count bigint,  --  NEW: Added child invoices count field
...
BEGIN
  ...
  SELECT
    ...
    --  NEW: Count child invoices (reinvoices) for this invoice
    (
      SELECT COUNT(*)::bigint
  ...
  -- Group by all invoice and order fields to enable COUNT aggregation
  GROUP BY
    i.id,
    i.dealer_id,
    i.invoice_number,
...
-- Comment on function
COMMENT ON FUNCTION get_invoices_with_filters IS 'Fetches invoices with filters, includes comments count and child invoices count for each invoice';
