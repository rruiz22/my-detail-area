-- Fase 3 Día 3: Management Dashboard - Estadísticas y Analytics (Corregido)
...
-- Crear índices para mejorar rendimiento de consultas de analytics (sin funciones volátiles)
...
-- Índice para invitaciones sin predicado volátil
CREATE INDEX IF NOT EXISTS idx_dealer_invitations_expires_accepted 
ON dealer_invitations(expires_at, accepted_at, created_at);

CREATE INDEX IF NOT EXISTS idx_dealer_invitations_created 
ON dealer_invitations(created_at) WHERE accepted_at IS NULL;