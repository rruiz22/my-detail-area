-- Fix Reports: Use correct date field per order type, add service filter, add total_volume
-- Sales/Service use due_date, Recon/CarWash use completed_at for reporting

-- =====================================================
-- 1. DROP EXISTING FUNCTIONS (to avoid "function name not unique" errors)
-- =====================================================
...
-- =====================================================
-- 2. CREATE GET_ORDERS_ANALYTICS (with service filter)
-- Uses due_date for sales/service, completed_at for recon/carwash
-- =====================================================
...
BEGIN
  ...
  -- Uses order_type to determine which date field to use for filtering
  WITH filtered_orders AS (
    SELECT
      ...
      CASE
        ...
        ELSE NULL
      END AS sla_met,
      -- Determine which date to use based on order_type
      CASE
    ...
    WHERE o.dealer_id = p_dealer_id
      ...
      -- Date filtering based on order_type
      AND (
      ...
      -- Service filter: check if the service exists in the services JSONB array
      AND (
  ...
  metrics AS (
    SELECT
      COUNT(*) AS total,
      -- Total volume: count all services across all orders
...
-- =====================================================
-- 3. CREATE GET_REVENUE_ANALYTICS (with service filter)
-- Uses due_date for sales/service, completed_at for recon/carwash
-- =====================================================
...
BEGIN
  ...
  -- Calculate period data using correct date field per order_type
  ...
  previous_period_calc AS (
    ...
    WHERE dealer_id = p_dealer_id
      AND status != 'cancelled'
      -- Date filtering for previous period based on order_type
      AND (
...
-- =====================================================
-- 4. CREATE GET_PERFORMANCE_TRENDS (with service filter)
-- Uses due_date for sales/service, completed_at for recon/carwash
-- =====================================================
...
-- Add comments explaining the date filtering logic
COMMENT ON FUNCTION get_orders_analytics IS 'Filters orders by due_date (sales/service) or completed_at (recon/carwash). Supports optional service filtering and returns total_volume (count of services).';
COMMENT ON FUNCTION get_revenue_analytics IS 'Uses due_date for sales/service, completed_at for recon/carwash. Supports optional service filtering.';
COMMENT ON FUNCTION get_performance_trends IS 'Uses due_date for sales/service, completed_at for recon/carwash. Supports optional service filtering.';
