...
-- This enables automatic creation of work items when adding new vehicles
...
CREATE POLICY "Admins and managers can create work item templates"
  ...
  WITH CHECK (
    dealer_id IN (
      ...
      AND p.role IN ('dealer_admin', 'dealer_manager', 'system_admin')
    )
  );
...
CREATE POLICY "Admins can delete work item templates"
  ...
  USING (
    dealer_id IN (
      ...
      WHERE dm.user_id = auth.uid()
      AND dm.is_active = true
      AND p.role IN ('dealer_admin', 'system_admin')
    )
  );
