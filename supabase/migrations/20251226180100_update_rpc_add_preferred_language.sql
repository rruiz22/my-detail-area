-- =====================================================
-- Update find_overdue_punches RPC to include preferred_language
-- =====================================================
-- This migration updates the RPC to return the employee's
-- preferred language for SMS reminders.
-- Also simplifies to single reminder (30 min) before auto-close (60 min)
-- =====================================================
...
RETURNS TABLE (
  ...
  employee_name TEXT,
  employee_phone TEXT,
  employee_preferred_language TEXT,
...
BEGIN
  ...
  WITH entry_details AS (
    SELECT
      ...
      COALESCE(emp.preferred_language, 'en') AS employee_preferred_language,
      te.dealership_id,
      te.clock_in,
      ...
      -- Single reminder at 30 minutes, auto-close at 60 minutes
  ...
  SELECT
    ...
    ed.employee_name,
    ed.employee_phone,
    ed.employee_preferred_language,
    ed.dealership_id,
    ed.clock_in,
    ed.shift_end_time,
    ed.shift_end_datetime,
    ed.minutes_overdue,
    -- Determine action: single reminder then auto-close
    CASE
      WHEN COALESCE(rs.reminder_count, 0) = 0
           AND ed.minutes_overdue >= ed.first_reminder_threshold
           AND ed.minutes_overdue < ed.auto_close_threshold
      THEN 'first_reminder'
...
Returns employee_preferred_language for localized SMS messages.
Uses single reminder system: reminder at 30 min, auto-close at 60 min (configurable).
No second reminder - simplified flow.';
