-- Migration: Fix order_type filter to use department field
-- Problem: RPC filters by metadata->>'order_type' but invoices store metadata->>'department'
-- Solution: Update WHERE clause to filter by metadata->>'department' instead
...
-- Recreate function with fixed department filter
...
BEGIN
  ...
  WHERE i.dealer_id = p_dealer_id::integer
    ...
    -- FIXED: Filter by department (stored in metadata->>'department')
    AND (
    ...
    -- Filter by tags (if p_tag_names is provided and not empty)
    AND (
...
-- Comment on function
COMMENT ON FUNCTION get_invoices_with_filters IS 'Fetches invoices with filters, includes comments count, child invoices count, tags, and supports filtering by department/order_type and tag names';
