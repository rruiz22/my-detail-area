-- Migration: Add tags filter to get_invoices_with_filters RPC
-- Problem: RPC does not support filtering by tags
-- Solution: Add p_tag_names parameter and filter invoices that have at least one of the specified tags
...
-- Recreate function with p_tag_names parameter
CREATE OR REPLACE FUNCTION get_invoices_with_filters(
  ...
  p_tag_names text[] DEFAULT NULL  -- NEW: Filter by tag names
)
RETURNS TABLE (
  id uuid,
  dealer_id integer,
...
BEGIN
  ...
  WHERE i.dealer_id = p_dealer_id::integer
    ...
    -- NEW: Filter by tags (if p_tag_names is provided and not empty)
    -- Invoice must have at least one of the specified tags
    AND (
...
-- Comment on function
COMMENT ON FUNCTION get_invoices_with_filters IS 'Fetches invoices with filters, includes comments count, child invoices count, tags, and supports filtering by tag names';
