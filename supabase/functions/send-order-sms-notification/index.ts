...
// REMOVED (2025-11-21): createDefaultSMSPreferences function
// Reason: Simplified Level 3 validation no longer uses user_sms_notification_preferences
// New architecture: Only validates user_preferences.notification_sms (global toggle)
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // System now only uses 3-level validation (Follower → Role → Preferences)
    // To re-enable: Uncomment checkNotificationRules() and applyDealerNotificationRules()

    console.log(`🔐 [${requestId}] Starting 3-level validation for order ${request.orderId}`);
    ...
    console.log(`\n✅ [${requestId}] 3-LEVEL VALIDATION COMPLETE: ${eligibleUsers.length} eligible users`);
    console.log(`🔍 [${requestId}] Eligible users:`, eligibleUsers.map(u => ({ id: u.id, name: u.name, phone: u.phone_number })));

    if (eligibleUsers.length === 0) {
      console.log(`⚠️ [${requestId}] NO ELIGIBLE USERS - Returning 0 sent`);
      return new Response(
        JSON.stringify({
          success: true,
          sent: 0,
    ...
    console.log(`📋 [${requestId}] LEVEL 4 SKIPPED (disabled): ${usersAfterDealerRules.length} users pass through`);
    ...
    console.log(`\n👤 [${requestId}] SELF-EXCLUSION CHECK:`);
    ...
    if (finalUsers.length === 0) {
      console.log(`⚠️ [${requestId}] ONLY TRIGGER USER - Returning 0 sent\n`);
      return new Response(
        JSON.stringify({
          success: true,
          sent: 0,
    ...
    console.log(`\n💬 [${requestId}] GENERATING MESSAGES...`);
    const messages = generateMessages(finalUsers, request.eventType, request.eventData);
    console.log(`✅ [${requestId}] Generated ${messages.length} messages`);
    messages.forEach(({ user, message }) => {
      console.log(`   → ${user.name} (${user.phone_number}): ${message.substring(0, 80)}...`);
    });

    console.log(`\n📤 [${requestId}] SENDING SMS VIA TWILIO...`);
    ...
    console.log(`\n💾 [${requestId}] RECORDING TO sms_send_history...`);
    ...
    console.log(`📊 Summary: ${successCount} sent, ${failedCount} failed`);
    console.log(`👥 Recipients: ${recipientNames.join(', ')}`);
    console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
...
async function getUsersEligibleForNotification(
  ...
  for (const follower of followers) {
    ...
    // SIMPLIFIED VALIDATION (2025-11-21): Only check global SMS toggle
    // Removed: user_sms_notification_preferences table validation
    // Removed: event_preferences granular control
    // New architecture: Level 3 = phone + global SMS toggle only

    const { data: userGlobalPrefs, error: globalPrefsError } = await supabase
      .from('user_preferences')
      .select('notification_sms')
      .eq('user_id', userId)
      .single();

    if (globalPrefsError || !userGlobalPrefs) {
      console.log(`   ❌ LEVEL 3 FAILED: No user preferences found`);
      continue;
    }
