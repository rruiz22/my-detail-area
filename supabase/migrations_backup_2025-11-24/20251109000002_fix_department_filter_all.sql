-- Fix department filtering when order_type = 'all'
-- This ensures Total Revenue matches Total by Departments by excluding invalid order_types
--
-- BUG: When filtering by 'all departments', the function was including orders with
-- order_type values outside of the 4 valid departments (sales, service, recon, carwash).
-- This created a discrepancy where Total Revenue > Sum of Individual Departments.
--
-- FIX: Explicitly restrict to ONLY the 4 valid departments when p_order_type = 'all'

DROP FUNCTION IF EXISTS get_revenue_analytics(INTEGER, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, TEXT, TEXT, TEXT[]);
...
BEGIN
  ...
  WITH period_aggregation AS (
    ...
    WHERE dealer_id = p_dealer_id
      ...
      -- FIX: When p_order_type = 'all', restrict to ONLY the 4 valid departments
      AND (
        (p_order_type = 'all' AND order_type IN ('sales', 'service', 'recon', 'carwash'))
      ...
      AND (
        ...
        (order_type IN ('recon', 'carwash') AND COALESCE(completed_at, created_at) BETWEEN p_start_date AND p_end_date)
      )
      AND (
  ...
  service_revenue AS (
    ...
    WHERE o.dealer_id = p_dealer_id
      ...
      AND (
        ...
        (o.order_type IN ('recon', 'carwash') AND COALESCE(o.completed_at, o.created_at) BETWEEN p_start_date AND p_end_date)
      )
  ...
  previous_period_calc AS (
    ...
    WHERE dealer_id = p_dealer_id
      ...
      AND (
        ...
        (order_type IN ('recon', 'carwash') AND COALESCE(completed_at, created_at) BETWEEN (p_start_date - (p_end_date - p_start_date)) AND p_start_date)
      )
      AND (
        p_service_ids IS NULL
        OR p_service_ids = '{}'
