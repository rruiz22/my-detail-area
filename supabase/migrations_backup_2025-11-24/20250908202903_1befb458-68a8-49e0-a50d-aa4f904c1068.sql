...
-- Update the existing get_dealer_services_for_user function to use the new system
CREATE OR REPLACE FUNCTION public.get_dealer_services_for_user(p_dealer_id bigint)
RETURNS TABLE(id uuid, dealer_id bigint, name text, description text, price numeric, category_name text, category_color text, duration integer, is_active boolean, created_at timestamp with time zone, updated_at timestamp with time zone, assigned_groups uuid[])
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = 'public'
AS $function$
