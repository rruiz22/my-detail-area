-- FASE 1: ReconHub Database Foundation (Fixed)
...
-- 6. Métricas T2L (Fixed - no generated columns)
...
-- Función para obtener estadísticas T2L de un dealer (con cálculos en runtime)
...
BEGIN
  ...
  SELECT 
    ROUND(AVG(
      CASE 
        WHEN t2l.frontline_ready_date IS NOT NULL 
        THEN EXTRACT(EPOCH FROM (t2l.frontline_ready_date - t2l.acquisition_date)) / 3600
        ELSE NULL 
      END
    ), 2) as average_t2l_hours,
    MIN(
      CASE 
        WHEN t2l.frontline_ready_date IS NOT NULL 
        THEN EXTRACT(EPOCH FROM (t2l.frontline_ready_date - t2l.acquisition_date)) / 3600
    ...
    ROUND(AVG(
      CASE 
        WHEN t2l.frontline_ready_date IS NOT NULL 
        THEN t2l.holding_cost_daily * EXTRACT(DAY FROM (t2l.frontline_ready_date - t2l.acquisition_date))
        ELSE t2l.holding_cost_daily * EXTRACT(DAY FROM (now() - t2l.acquisition_date))
      END
    ), 2) as average_holding_cost
