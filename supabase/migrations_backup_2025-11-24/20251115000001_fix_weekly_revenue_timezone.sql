-- Fix weekly revenue grouping to use America/New_York timezone
-- This ensures weeks are Monday-Sunday based on New York time, not UTC
...
BEGIN
  ...
  WITH period_aggregation AS (
    SELECT
      -- Convert to New York timezone before formatting the period
      TO_CHAR(
        (CASE
...
-- Add helpful comment explaining the timezone conversion
COMMENT ON FUNCTION get_revenue_analytics IS 'Revenue analytics with timezone-aware weekly grouping. Weekly periods are Monday-Sunday based on America/New_York timezone (EST/EDT). Format: IYYY-IW (e.g., "2024-45" = Week 45 of 2024)';
