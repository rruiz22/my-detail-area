-- =====================================================
-- FIX: Use TOTAL accumulated time in step (previous + current)
-- Date: 2025-10-16
-- Issue: Only counting current visit, should count ALL time in step
-- =====================================================
...
BEGIN
  ...
  WITH vehicle_total_time AS (
    ...
    SELECT
      v.id as vehicle_id,
      v.step_id,
      COALESCE(vst.total_accumulated_hours / 24.0,
               EXTRACT(EPOCH FROM (NOW() - v.intake_date)) / 86400) as total_days_in_step
  ...
  vehicle_counts AS (
    SELECT
      ...
      COUNT(CASE WHEN vt.total_days_in_step >= 2 AND vt.total_days_in_step <= 3 THEN 1 END) as two_three_days,
